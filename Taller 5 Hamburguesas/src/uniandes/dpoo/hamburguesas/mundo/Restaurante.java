...
public class Restaurante
{
    ...
    public String mostrarMenu(){
        ...
        menu.append("---- Menu de Productos Básicos ----\n");
        for (ProductoMenu producto : menuBase){
            menu.append(producto.getNombre() + " - $" + producto.getPrecio() + "\n");
        }

        menu.append("---- Menu de Combos  ----\n");
        for (Combo combo : menuCombos){
            menu.append(combo.getNombre() + " - $" + combo.getPrecio() + "\n" + "Descuento: " + combo.getDescuento() + "\n");
        }

        return menu.toString();
    }

    public void agregarProductoAPedido(String nombreProducto, ArrayList<Ingrediente> ingredientesAgregar, ArrayList<Ingrediente> ingredientesEliminar) throws NoHayPedidoEnCursoException {
        if (pedidoEnCurso == null) {
            throw new NoHayPedidoEnCursoException();
        }
        ...
        // Si no hay ajustes, se agrega directamente el producto base
        if (ingredientesAgregar.isEmpty() && ingredientesEliminar.isEmpty()) {
            pedidoEnCurso.agregarProducto(productoSeleccionado);
        } else {
            // Crea un producto ajustado y agrega ingredientes
            ProductoAjustado productoAjustado = new ProductoAjustado(productoSeleccionado);
            for (Ingrediente ing : ingredientesAgregar) {
                productoAjustado.getAgregados().add(ing);
            }
            for (Ingrediente ing : ingredientesEliminar) {
                productoAjustado.getEliminados().add(ing);
            }
